...
class cat1(object):
    def activate(resp,intent_index):
        node_type=resp['intents'][intent_index]['name'].split('_')[1]
        print(node_type.title())
        #concatenate node type with attribute (positionType)
        returned_attribute=node_type+""+resp['intents'][intent_index]['name'].split('_')[2]
        print(returned_attribute)

        constraints_attribute,constraints_value=get_Constraints(resp)
        returned_attribute=get_returned_attributes(resp,node_type)
        print(constraints_attribute)
        print(constraints_value)

        #title func to turn the first letter uppercase
        execute_query(node_type.title(),returned_attribute,constraints_attribute,constraints_value)
